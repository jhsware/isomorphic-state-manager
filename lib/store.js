...
Store.prototype.addSubscriber = function (callback, thisArg) {
    ...
    for (var i = 0; i < this.subscribers.length; i++) {
        exists = Array.isArray(this.subscribers[i]) && this.subscribers[i][0] === callback && this.subscribers[i][1] === thisArg;
        if (exists) break;
    }
...
Store.prototype.removeSubscriber = function (callback, thisArg) {
    for (var i = 0; i < this.subscribers.length; i++) {
        if (this.subscribers[i] === undefined) {
            continue;
        } else if (this.subscribers[i][0] === callback && this.subscribers[i][1] === thisArg) {
            // Mark for deletion
    ...
    */
    ...
    for (var i = 0; i < this.subscribers.length; i++) {
        if (this.subscribers[i] !== undefined) {
            ...
            try {
                this.subscribers[i][0].call(this.subscribers[i][1])
            } catch (e) {
